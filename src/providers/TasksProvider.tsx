...
import { type Modal, type ModalsState, type TasksContextResponse, type TasksState } from "./TasksProvider.types"
...
const TasksProvider = ({ children }: TasksProviderProps) => {
  ...
  const [modalsOpened, setModalsOpened] = useState<ModalsState>({ addTask: false, editTask: false, deleteTask: false })
  ...
  const closeModal = (modal: Modal) => setModalsOpened((currentModalsOpened) => ({
    ...currentModalsOpened,
    [modal]: false
  }))

  const handleAddTaskClick = useCallback((taskInput: TaskInput) => {
    const taskId = uuidv4()
    const updatedTasks = {
      [taskId]: {
        id: taskId,
  ...
  const handleEditTaskClick = useCallback((taskInput: TaskInput, taskId: string) => {
